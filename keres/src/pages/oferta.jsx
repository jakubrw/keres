...
const Oferta = () => {
	...
	const offersArr = [
		{
			cardTitle: 'doradztwo',
			cardText: 'Nasze zdobyte doświadczenie daje możliwość w intuicyjny sposób wybrać najkorzystniejsze opcje. Stała kontrola przebiegu transportu pozwala przewidzieć i uniknąć błędów, które mogłyby się pojawić.',
		},
		{
			cardTitle: 'odprawy',
		...
		{
			cardTitle: 'spedycja',
			cardText: 'Działalność spedycyjna daje nam możliwość szybkiej realizacji importu/exportu z dowolnego miejsca. Stała współpraca z przewoźnikami oferuje nam pewność bezpiecznej i terminowej realizacji.',
		},
		{
			cardTitle: 'transport',
			cardText: 'Nasze realizacje są obsługiwane 24h/7, jeśli załadunki występują wieczorową porą są również przez nas kontrolowane, terminowe realizacje to dla nas priorytet. Posiadamy niezbędne uprawnienia do przewozu towarów niebezpiecznych, w związku z tym mogą nam Państwo powierzyć kontrolę nad realizacją ładunku.',
		},
		{
			cardTitle: 'ubezpieczenia',
			cardText: 'Wszystkie nasze realizacje są ubezpieczone, począwszy od transportów naszą flotą, skończywszy na realizacji transportów spedycyjnych. Dodatkowo, na prośbę klienta jesteśmy w stanie zorganizować ubezpieczenie cargo, dzięki wyłączności agenta takie tematy jesteśmy w stanie zrealizować na bieżąco.',
		},
	]

	return (
		<div className='min-h-screen w-full relative bg-gradient-to-r from-gray-100 to-gray-200'>
			<Header
				logoWidth={30}
				logoHeight={30}
				titleName={'Oferta'}
			...
			<div className='flex flex-col items-center gap-3'>
				<div className='w-full md:hidden'>
					...
					>
						{offersArr.map((x, index) => (
							<SwiperSlide
								key={index}
								className='text-black font-solid flex flex-col items-space justify-evenly overflow-hidden rounded-xl bg-gradient-to-r from-blue-200 to-blue-400 p-6 shadow-lg'>
								<CardOffert key={index} cardTitle={x.cardTitle} cardText={x.cardText} />
							</SwiperSlide>
						))}
					</Swiper>
				</div>
				<div className='hidden aspect-[4/2] w-full md:block md:w-[70vw] overflow-hidden lg:aspect-[5/2]'>
					<div className='flex flex-row items-center justify-center h-full bg-gradient-to-r from-yellow-light to-green-dark overflow-hidden rounded-xl shadow-xl'>
						{offersArr.map((x, index) => (
							<motion.div
								key={index}
								className={`h-full ${expandedCard === index ? 'w-full' : 'w-1/5'} flex flex-col items-center justify-center text-black font-solid flex flex-col items-space justify-evenly overflow-hidden rounded-xl p-6 cursor-pointer border border-gray-300`}
									...
									{expandedCard === index ? (
										<motion.div
											...
											<Link
												...
												className='w-full bg-gradient-to-r from-teal-500 to-green-500 text-white text-center py-2 px-4 rounded-lg shadow-md hover:from-teal-600 hover:to-green-600 transition duration-200 w-1/3'>
												Kliknij po więcej
											</Link>
										</motion.div>
									) : (
				...
				<div className='px-4 md:w-[70vw] mt-6 bg-white bg-opacity-60 p-6 rounded-lg shadow-md'>
